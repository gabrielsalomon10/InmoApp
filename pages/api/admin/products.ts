...
const updateProduct = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        if( !product ) {
            await db.disconnect();
            return res.status(400).json({ message: 'No extiste un producto con ese ID'});
        }
...
const createProduct = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if( images.length < 1 ) {
        return res.status(400).json({ message: 'Debe tener minimo 1 imagen'});
    }
    ...
    try {
        ...
        if( productInDB ) {
            return res.status(400).json({ message: 'Ya existe una propiedad con ese slug'});
        }
